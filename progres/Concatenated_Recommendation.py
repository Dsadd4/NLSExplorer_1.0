...
#去除单个位点的预测结果
base_dir = '/data/liyifan/progres/193_important_segment_without_single'
have_struc = load_mydict('/data/liyifan/progres/NLS_protein_with_alphafold')
...
class gnn_classifier(nn.Module):
    ...
    def forward(self, embding,coord,adj_matrix,positive_for_train,negative_for_train,preseg,mode):
            ...
            if coord!= [] and choose == 'include-struc' and edge_emb.size()[1]==embding.size()[1]:
                ...
                for item in preseg:
                    ...
                    if pre == 'begjin':
                        ...
                        pre =torch.concat((pre,pre_di),1)
                        embding_li.append(sum_)
                        
                return pre,embding_li   
            else:
...
#最新推荐出来的数据
def generate_single_training(name,label,pre_seg,chosee):
    import os
    ...
    q = f"{name}.pdb"
    if chosee != "large":
        dir = '/data/liyifan/progres/183_alphafold_pdb/'+q
    else:
        dir = '/data/liyifan/progres/loc_in_nucleus_with_pdb/'+q
    #--------------------------------------------------------------------------------------------------------
    #在这里判断是否有对应文件
...
def get_data(insp_train,choose):
    seq_store = []
    data_store = []
    label_store = []
    segment_store = []
    ...
    faied_li = []
    # print(insp_train)
    f = open('/data/liyifan/progres/failed_gai_pdb.fasta','w')
    for st_d in insp_train:
        ...
        ins_ac = st_d[2]
        ins_pre = st_d[3]
        ac_store.append(ins_ac)
        segment_store.append(ins_pre)
        # try:
        #     data_for_train = generate_single_training(ins_ac,ins_label ,ins_pre)
        # except:
        #     faied_li.append(ins_ac)
        #     continue
        data_for_train = generate_single_training(ins_ac,ins_label ,ins_pre,choose)
        ...
        if data_for_train[0]==[]:
            ...
            f.write(ins_seq)
            f.write('\n')
            f1 = open(f"/data/liyifan/progres/结构待alphafold计算/{ins_ac}.fasta",'w')
            f1.write('>'+ins_ac+'\n')
            # print(ins_seq)
            # print('---------------------------------------')
            f1.write(ins_seq)
    ...
    return data_store,label_store,tot_emb,segment_store,seq_store,ac_store



def train_model(data_store,tot_emb,model):
    for k in range(1):
        ...
        for epoch in range(25):
            ...
            for i,ba_data in enumerate(data_store):
                ...
                pre = model(tot_emb[i],ba_data[0],ba_data[2],po_for_train,nefor_t,[],'train')
                ...
                loss.backward()
                optimizer.step()
            torch.save(model.state_dict(), f'/data/liyifan/progres/resconn_{epoch}_nls_model_not_include')
...
# insp_train = insp_train0+insp_train1+insp_train2+insp_train3

# data_store,label_store,tot_emb,_,__,___= get_data(insp_train,'nl')

 

model = gnn_classifier().cuda() 
# model = train_model(data_store,tot_emb,model)
Path_ = "/data/liyifan/progres/resconn_20_nls_model_not_include"
...
def seg2seq(seq,seg):
    re = []
    for sg in seg:
        re.append(seq[sg[0]:sg[1]+1])
    return re


def get_recom_seg(X_te,embding_te,model,pre_seg,seq_store,ac_store,top=0):
    #默认一次数据只有一条nls
    result = []
    import torch
    with torch.no_grad():
        ...
        for j,ba_data in enumerate(X_te):
            ...
            print(len(seq))
            print('-------------------gscz2---------------')
            print(embding_te[j].size())
            print('-------------------ttttt---------------')
            max_len =embding_te[j].size()[1]
            
            segg = corr_r(segg,max_len)
            
            
            # print(segg)
            pre,embding_li = model(embding_te[j],ba_data[0],ba_data[2],[],[],segg,'prediction' )
            # print(pre)
            possibilty = torch.sort(pre, descending=True)
            ...
            rec_c = min(top,len(recomd[0]))
            for p_i in recomd[0][0:rec_c]:
                seg_3.append(segg[p_i])
                recomd_emb.append(embding_li[p_i])
            
            seq_seg = seg2seq(seq,seg_3)
            
            single_result = [acc,seq,seq_seg,seg_3,recomd_emb]
            # print(single_result)
            result.append(single_result)
    return result,possibilty
  

#最新推荐出来的数据
# insp_train = load_mydict('./for_recom/hybrd2_0.31')
import os
import sys
...
for nd in names_d:
    print(nd[:-4])
    insp_train = load_mydict(f'./progres/recomenda_trans/{nd[:-4]}')
    
    print(insp_train[0])
    #这个数据内标签每个都是 (1,3)，不会用到只是为了计算方便，
    

    data_store2,label_store2,tot_emb2,segment_store_2,seq_store2,ac_store2 = get_data(insp_train,"large")
    
    from utils import save_mydict,load_mydict
    result,possbi = get_recom_seg(data_store2,tot_emb2,model,segment_store_2,seq_store2,ac_store2,topp)
    print(len(data_store2))
    re_without_emb = [item [:-1] for item in result]
    path = './progres/result_save'
    print("saving......")
    save_mydict(result,path+f'/{nd[:-4]}')

print(re_without_emb)
print(f"the possibilty sort is:{possbi}")
save_mydict([re_without_emb[0][0],re_without_emb[0][-1]],'./DANN_union/vis_trans/last_recom')



def get_recom_alnls_seg(X_te,embding_te,model,pre_seg,seq_store,ac_store,top=0):
    #默认一次数据只有一条nls
    result = []
    import torch
    with torch.no_grad():
...
import requests
import re
def get_protein_Organism(protein_id):
    # UniProt API的URL
...
#         print(data)
        ...
        organism = (re.sub(r'\([^)]*\)', '', organism)).strip()
        print(f"Organism: {organism}")
        return organism
    else:
        print(f"Error: Unable to retrieve information for {protein_id}")
        return None

# # 用示例蛋白质序列名字调用函数
# protein_id = "A1L3G9"
# get_protein_Organism(protein_id)



# insp_train = load_mydict(f'./for_recom/fulld_0.3')
# os_li = []
# nls_with_os = []
# for item in insp_train[0:10]:
#     ac = item[2]
#     os = get_protein_Organism(ac)
#     if os!=None:
#         os_li.append(os)
#         temp = item
#         #这里把正确标签放到要预测的地方就可以了
#         temp[3]=[(temp[1][0]-1,temp[1][1]-1)]
#         nls_with_os.append(temp)
    
    
# data_store2,label_store2,tot_emb2,segment_store_2,seq_store2,ac_store2 = get_data(nls_with_os,"large")

# result = get_recom_seg(data_store2,tot_emb2,model,segment_store_2,seq_store2,ac_store2,3)

# for item in result:
#     print(str(item[0])+':'+str(item[3])+','+str(item[2]))
# from utils import save_mydict
# path = './多物种推荐结果/real_NLS_location/'
# print("saving......")
# save_mydict(result,path+f'real_nls')
# save_mydict(os_li,path+f'os_nls')
