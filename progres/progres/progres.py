...
class Model(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        return normalize(out, dim=1),extract_feat,extract_adj_mat,extract_mask
...
def embed_graph(graph, device="cpu"):
    with torch.no_grad():
        ...
        for batch in data_loader:
            emb = model(batch.to(device))[0]
            break
        return emb.squeeze(0)

def embed_feature(graph, device="cpu"):
    with torch.no_grad():
        model = load_trained_model(device)
        data_loader = DataLoader([graph], batch_size=1)
        for batch in data_loader:
            emb = model(batch.to(device))[1]
            break
        return emb.squeeze(0)

def embed_coords_all(coords, device="cpu"):
    graph = coords_to_graph(coords)
    return embed_feature(graph, device)

def embed_feature_2(graph, device="cpu"):
    with torch.no_grad():
        model = load_trained_model(device)
        data_loader = DataLoader([graph], batch_size=1)
        for batch in data_loader:
            # emb = model(batch.to(device))[1]
            extract_feat,extract_adj_mat,extract_mask = model(batch.to(device))[1:]
            break
        return extract_feat,extract_adj_mat,extract_mask 

def get_ecgnn_feature(coords, device="cpu"):
    graph = coords_to_graph(coords)
    return embed_feature_2(graph, device)
